    ...
    <div className="absolute inset-y-0 right-0 w-[400px] bg-[#FAF7F0] shadow-2xl flex flex-col z-40 anim-slide-right border-l border-[#EDE4D4]">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-6">
        ...
        <div className="space-y-3 mb-8">
          {faqs.map((faq, idx) => (
            <div key={idx} className="border border-[#EDE4D4] rounded-xl overflow-hidden bg-white">
              <button 
                onClick={() => setOpenFaq(openFaq === idx ? null : idx)}
                className="w-full flex justify-between items-center p-4 text-left font-bold text-[#1A1208] hover:bg-[#F2ECE0]"
              >
                <span>{faq.q}</span>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-4 bg-[#F2ECE0] rounded-xl">
