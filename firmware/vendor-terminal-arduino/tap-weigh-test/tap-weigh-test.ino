/*
 * Tap-Weigh-Tap Session Test — Arduino IDE build
 ...
 * vendor_id, food_id, api_url, auth_token. This board has already been provisioned with
 * these values — no need to re-run the provisioning sketch.
 *
 * Arduino IDE setup:
 ...
 *        - HX711 by Bogdan Necula (bogde) (v0.7.x)
 *        - ArduinoJson by Benoit Blanchon (v6.21.x — NOT v7)
 *      (WiFi, HTTPClient, Preferences, BLEDevice ship with the ESP32 board package — no install needed)
 *   3. Tools menu:
 *        - Board: "ESP32 Dev Module"
 *        - Partition Scheme: "Huge APP (3MB No OTA/1MB SPIFFS)"  <-- required, BLE+WiFi+HTTPClient+ArduinoJson
 *                                                                      overflow the default partition
 *        - Upload Speed: 921600
 *        - Port: whichever COM port the ESP32 shows up on
 *   4. Click Upload. Hold the board's BOOT button when it shows "Connecting....",
 *      release once upload starts.
 *
 * Pins:
