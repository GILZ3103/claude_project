...
// ── BLE beacon config ─────────────────────────────────────────────────────────
// Must match useLivePosition.ts VENUE_SERVICE_UUID. Change ANCHOR_MINOR per node.
#define VENUE_SERVICE_UUID  "6e2c0000-b85e-4f3a-9c1d-2a7f5e8d4b10"
#define ANCHOR_MINOR        1      // unique per stall; add matching row in positioning_anchors
...
// ── Serial "button" — type any key in Serial Monitor to simulate button press ─
void handleSerialButton() {
    if (Serial.available()) {
...
void setup() {
    ...
    // BLE beacon — advertises this stall's anchor ID for phone-side trilateration
    ...
    state          = IDLE;
    stateEnteredAt = millis();
    Serial.println("System Ready — press button (or send any key via Serial) to start weighing");
}
