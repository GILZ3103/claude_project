...
const enrolSchema = zod_1.z.object({
    card_uid: zod_1.z.string().min(4).max(20)
});
const kioskTapSchema = zod_1.z.object({
    card_uid: zod_1.z.string().min(4).max(20),
    kiosk_id: zod_1.z.string().uuid(),
...
router.get('/', async (req, res) => {
    ...
    const { data: progressRows } = await supabase_1.supabase
...
router.post('/:id/enrol', (0, validate_1.validate)(enrolSchema), async (req, res) => {
    const { id } = req.params;
    ...
    const { data: card } = await supabase_1.supabase.from('cards').select('uid').eq('uid', card_uid).single();
    if (!card) {
...
router.post('/kiosk/tap', (0, validate_1.validate)(kioskTapSchema), async (req, res) => {
    ...
    // Validate kiosk
    const { data: kiosk } = await supabase_1.supabase.from('kiosks').select('*').eq('kiosk_id', kiosk_id).eq('is_active', true).single();
    if (!kiosk) {
    ...
    const { data: tapEvent } = await supabase_1.supabase
        .from('tap_events')
        .insert({
        card_uid,
        vendor_id: null,
    ...
    let points_added = 0;
    try {
        const { data: drCampaigns } = await supabase_1.supabase
            .from('campaigns')
            .select('*')
            .eq('is_active', true)
        ...
        for (const campaign of drCampaigns ?? []) {
            ...
            await supabase_1.supabase.from('cards').update({
