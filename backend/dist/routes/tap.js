...
const tapSchema = zod_1.z.object({
    card_uid: zod_1.z.string().min(4).max(20),
    vendor_id: zod_1.z.string().uuid(),
    food_id: zod_1.z.string().uuid(),
    device_timestamp: zod_1.z.string(),
    synced_from_queue: zod_1.z.boolean().default(false),
    weight_g: zod_1.z.number().positive().optional()
});
const syncSchema = zod_1.z.object({
    terminal_mac: zod_1.z.string().regex(/^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$/),
...
async function processTap(card_uid, vendor_id, food_id, device_timestamp, synced_from_queue, weight_g) {
    // 1. Fetch card
    const { data: card } = await supabase_1.supabase.from('cards').select('*').eq('uid', card_uid).single();
    if (!card)
        return { status: 404, body: { success: false, error: 'CARD_NOT_FOUND', message: 'Card not found.' } };
    if (!card.is_active)
        return { status: 403, body: { success: false, error: 'CARD_INACTIVE', message: 'Card is inactive.' } };
    // 2. Fetch vendor
    const { data: vendor } = await supabase_1.supabase.from('vendors').select('*').eq('vendor_id', vendor_id).single();
    if (!vendor)
        return { status: 404, body: { success: false, error: 'VENDOR_NOT_FOUND', message: 'Vendor not found.' } };
    // 3. Fetch food item
    const { data: food } = await supabase_1.supabase.from('food_items').select('*').eq('food_id', food_id).single();
    if (!food)
        return { status: 404, body: { success: false, error: 'FOOD_NOT_FOUND', message: 'Food item not found.' } };
    if (food.vendor_id !== vendor_id)
        return { status: 400, body: { success: false, error: 'FOOD_NOT_FOUND', message: 'Food item does not belong to this vendor.' } };
    // 4. Duplicate tap check
    const today = new Date().toISOString().split('T')[0];
    const checkDate = synced_from_queue ? device_timestamp.split('T')[0] : today;
    const { data: existingTap } = await supabase_1.supabase
        .from('tap_events')
        .select('event_id')
        ...
        .single();
    if (existingTap)
        return { status: 409, body: { success: false, error: 'DUPLICATE_TAP', message: 'Card already tapped at this vendor today.' } };
    // 5. Voucher lookup
    let voucher = null;
    const { data: vouchers } = await supabase_1.supabase
        .from('vouchers')
        .select('*')
        .eq('card_uid', card_uid)
        .eq('status', 'ACTIVE')
    ...
    const calories = (weight_g && food.calories_per_100g)
    ...
    // a. Deduct points
    const { error: balanceErr } = await supabase_1.supabase
        .from('cards')
    ...
    // b. Mark voucher used
    if (voucher) {
        await supabase_1.supabase.from('vouchers').update({
            status: 'USED',
    ...
    // c. Insert tap event
    const { data: tapEvent, error: tapErr } = await supabase_1.supabase
        .from('tap_events')
        .insert({
        card_uid,
        vendor_id,
    ...
    let campaign_completed = null;
    let voucher_issued = null;
    try {
        const { data: activeCampaigns } = await supabase_1.supabase
            .from('campaigns')
            .select('*')
            .eq('is_active', true)
        ...
        for (const campaign of activeCampaigns ?? []) {
            ...
            if (campaign.condition_type === 'SPEND_POINTS') {
                ...
                const { data: alreadyVisited } = await supabase_1.supabase
                    .from('tap_events')
                    .select('event_id')
                    .eq('card_uid', card_uid)
                    .eq('vendor_id', vendor_id)
            ...
            await supabase_1.supabase.from('campaign_progress').update({
                current_value: new_value,
                completed,
                completed_at: completed ? server_timestamp : null
            }).eq('progress_id', progress.progress_id);
            if (completed) {
                const { data: newVoucher } = await supabase_1.supabase
                    .from('vouchers')
                    .insert({
                    card_uid,
    ...
    // 11. Calorie warning
    const { data: todayTaps } = await supabase_1.supabase
        .from('tap_events')
        .select('metadata')
        .eq('card_uid', card_uid)
        .eq('event_type', 'TAP_PURCHASE')
...
// POST /api/tap
router.post('/', requireTerminalAuth, (0, validate_1.validate)(tapSchema), async (req, res) => {
...
// POST /api/tap/sync
router.post('/sync', requireTerminalAuth, (0, validate_1.validate)(syncSchema), async (req, res) => {
    ...
    const sorted = [...events].sort((a, b) => new Date(a.device_timestamp).getTime() - new Date(b.device_timestamp).getTime());
    let processed = 0;
    let skipped = 0;
    for (const event of sorted) {
        const result = await processTap(event.card_uid, vendor.vendor_id, event.food_id, event.device_timestamp, true, event.weight_g);
